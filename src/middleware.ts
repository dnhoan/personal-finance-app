...
export const config = {
  ...
  // /api/webhooks is excluded for the same reason: SePay posts with no session
  // cookie and authenticates with its own Apikey header, and a 302 there would
  // burn all 7 delivery retries without leaving a trace to debug from. The PWA
  ...
  matcher: [
    "/((?!_next/static|_next/image|api/auth|api/cron|api/webhooks|sign-in|unauthorized|offline|sw.js|manifest.json|icons|favicon.ico|manifest.webmanifest|icon|apple-icon|robots.txt|sitemap.xml).*)",
  ],
};
